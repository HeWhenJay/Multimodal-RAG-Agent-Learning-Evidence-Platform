...
def parse_resume_template_docx(content: bytes, filename: str, template_id: str | None = None, version: int = 1) -> ResumeTemplateParseResponse:
    ...
    return ResumeTemplateParseResponse(
        ...
        layoutFingerprint=layout_fingerprint(content),
    )
...
def apply_resume_patches_to_docx(
    ...
    if detect_unsupported_regions(content):
        raise ValueError("简历包含暂不支持的复杂结构，拒绝自动导出")
...
def paragraph_style_fingerprint(paragraph: Paragraph) -> dict[str, Any]:
    ...
    return {
        "styleName": paragraph.style.name if paragraph.style is not None else None,
        ...
        "runs": [
            {
                ...
                "styleName": run.style.name if run.style is not None else None,
            }
...
def _is_heading(paragraph: Paragraph) -> bool:
    ...
    style_name = paragraph.style.name.lower() if paragraph.style is not None and paragraph.style.name else ""
