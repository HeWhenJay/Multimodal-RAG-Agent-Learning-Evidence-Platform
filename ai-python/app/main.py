...
from app.api.dsh_local_sync import router as dsh_local_sync_router
...
async def handle_request_validation_error(request: Request, error: RequestValidationError):
    ...
    if request.url.path.startswith(("/api/auth", "/api/rag", "/api/dsh-plugin", "/api/dsh-local-sync", "/api/agent", "/api/reviews")):
...
app.include_router(dsh_local_sync_router)
