...
分段任务会透传 evidence 清洗、LangExtract、实际模型、模型第 N 轮生成、单请求超时、剩余总预算、质量校验、自动修复和多卡复查事件，并每 15 秒刷新后台心跳。单段总执行预算由 `REVIEW_SEGMENT_TIMEOUT_SECONDS` 控制，默认 1800 秒；所有节点共享同一截止时间，超时后不再发起后续模型请求。交互式单次模型请求默认最多 240 秒，Cockpit 失败后快速切换 DeepSeek；某段超时只返回该段 `FAILED`，其他段继续完成。前端超过 60 秒未收到心跳时展示失联诊断，并允许用 `forceRestart=true` 显式替代旧任务。
...
当单卡 evidence 门禁已通过、但多卡复查或合并未收敛时，`ReviewSegmentResult.candidateAvailable=true`，`cards` 会携带最后一次完整有效候选；该候选默认不参与最终合并，由用户选择“人工保留候选 / 标准门禁重试 / 宽松门禁重试”，并可逐张勾选、编辑、删除或放弃。`candidateAvailable=false` 表示没有可安全交给用户审核的候选，仍可选择标准门禁或宽松门禁重试。关闭工作台不会修改正式卡片；只有最终合并才按用户勾选的全部候选整体替换当前正式卡片。
...
| `REVIEW_SEGMENT_REQUEST_TIMEOUT_SECONDS` | `240` | 交互式分段单次模型请求上限；必须明显小于单段总预算 |
| `REVIEW_SEGMENT_COCKPIT_REQUEST_RETRIES` | `0` | 交互式分段的 Cockpit 请求重试数；默认失败后直接切换 DeepSeek |
