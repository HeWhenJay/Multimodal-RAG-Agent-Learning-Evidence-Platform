...
- 迁移 Python Outbox 发布器时，Java `evidence.rag.kafka.outbox.publisher-enabled` 与 Python `RAG_OUTBOX_PUBLISHER_ENABLED` 不能同时开启。切换到 Python 后，将前者设为 `false`、后者设为 `true`；Java 仍可继续写入 `rag_outbox_event`，但不再发布它。
...
### Python Outbox cron 运行契约

Python cron 不新增 HTTP 接口。`ai-python/run.py` 读取配置后会在独立子进程中启动 `app.workers.scheduler`；cron 与 FastAPI API 进程隔离，Uvicorn reload 仅重启 API 子进程，不会重复执行定时发布。
...
| `RAG_KAFKA_PUBLISH_TIMEOUT_MS` | `3000` | 单条 Outbox 等待 Kafka 确认的最长时间，最小为 `100`。 |
| `RAG_STAGING_CLEANUP_ENABLED` | `false` | 是否启用过期 staging 索引清理任务。 |

切流顺序：先部署 Python worker 与 cron，验证其 PostgreSQL/Kafka 连通性；再将 Java `EVIDENCE_RAG_KAFKA_OUTBOX_PUBLISHER_ENABLED=false`、Python `RAG_OUTBOX_PUBLISHER_ENABLED=true` 同步切换；最后用一条新的索引任务核对 Outbox 状态和 Kafka 消费。若需回滚，先关闭 Python 发布任务，再恢复 Java 发布器，禁止两个发布者同时运行。
