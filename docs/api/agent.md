...
Python 内部入口统一为 `agents.orchestration.pae_react_graph`，FastAPI `/internal/agent/tasks` 调用 `start_unified_agent()`，`/internal/agent/tasks/{taskId}/resume` 调用 `resume_unified_agent()`。旧 `read_only_graph` 和 `planning_graph` 的确定性工具参数、草稿生成、候选填充值生成和 mutation 执行辅助函数仍被复用，但运行时主图以统一节点为准。

```mermaid
flowchart TD
    START["Start<br/>Java 创建 agent_task"] --> M0["记忆预取 1<br/>偏好、历史约束、近期任务"]
    M0 --> PL["规划器 Planner<br/>PAE 计划和完成标准"]
    PL --> PLAN_GATE{"是否需要计划确认"}
    PLAN_GATE -->|"是"| PLAN_REVIEW["HITL：PLAN 审批"]
    PLAN_GATE -->|"否"| M1["记忆预取 2<br/>按计划检索任务记忆"]
    PLAN_REVIEW -->|"批准"| M1
    PLAN_REVIEW -->|"拒绝"| ANSWER["回答节点"]
    M1 --> EXEC["执行器 Executor<br/>ReAct 行动选择"]
    EXEC --> TOOL["工具节点 Tool Adapter<br/>Java Gateway / 后续 MCP"]
    TOOL --> RESULT{"工具结果"}
    RESULT -->|"失败"| REPAIR["修补节点<br/>重试、跳过、重规划、无法完成"]
    REPAIR -->|"重试"| TOOL
    REPAIR -->|"重规划"| PL
    ...
    ACCEPT -->|"未完成"| EXEC
    ACCEPT -->|"通过"| ANSWER
    ANSWER --> DONE["任务响应<br/>完成、失败或等待输出审批"]
    ANSWER -. "显式开启或触发词" .-> MEM["长期记忆整理<br/>候选、冲突、版本建议"]
    MEM --> MEM_REVIEW["记忆写入审批<br/>CRUD / MEMORY_WRITE 语义"]
    MEM_REVIEW --> SAVE["Java Mutation Gateway<br/>agent_memory_candidate_save"]
```
...
| 记忆预取 2 | `memory_prefetch_after_planner` | 规划通过后按计划步骤再次检索任务相关记忆，并与第一阶段记忆去重 |
...
| 长期记忆整理 | `post_answer_memory` | 仅在显式开启或触发“记住/偏好/以后请”等语义时提炼候选，不自动激活 |
...
- 回答后的 `agent_memory_candidate_proposer` 只产出待确认候选和冲突信息，不写库、不激活。
- `agent_memory_candidate_save` 属于数据库变更工具，必须走 `human_crud_review`；前端可展示为 `MEMORY_WRITE` 语义，但底层审批、归属、版本和审计仍由 Java `CRUD` 流程处理。
...
| `resume_template_fill` | `resumeTemplateId/contentMap` | 仅保留为草稿候选字段，不作为 Tool Gateway 工具；Agent 不读写 DOCX，用户可参与的 DOCX 修改走 `/api/rag/resume/templates/*` |
...
当前简历修改不再让 Agent 直接决定可改范围，也不允许 Agent 直接读写 DOCX。用户先在 `/resume-template` 完成图片预览、区域确认和约束保存，再由 Java 根据 `useConfirmedAnnotations=true` 冻结允许修改的字段边界并生成补丁草稿。旧的 `resume_template_fill` 仅作为候选字段兼容说明，不作为当前前端入口：
...
- `resume_template_fill` 仅保留历史兼容，只返回待确认 `contentMap`，不读取模板路径、不生成 `outputPath`。
