...
4. 规划任务可经 `resume_rewrite_decision -> resume_rewrite_planner -> resume_rewrite_generator -> resume_rewrite_acceptance` 进入简历改写子图。该子图只产生待确认候选，不直接写入 DOCX 或业务数据。
...
7. `answer_writer` 产生带 evidence 的最终回答或输出草稿。规划输出需确认时写入 `WAITING_OUTPUT_REVIEW`；批准后的保存类操作还会进入 `WAITING_CRUD_REVIEW`，并携带审批、操作快照和幂等键。
