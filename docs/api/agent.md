...
当 `planning_task.input.resumeRewriteRequested=true`，或任务目标包含“修改简历”“优化简历”等意图时，输入应至少提供 `jobDescription` 和用户已授权读取的 `resumeText`。可选字段包括 `metadataFilter`、`topK`、`resumeTemplateId` 与 `resumeTemplateVersion`；服务端仍从任务记录推导当前用户，忽略输入中的 `userId`。

简历改写结果只作为 `WAITING_OUTPUT_REVIEW` 的候选草稿返回；以下对象位于规划草稿的 `resumeRewrite` 字段内：
...
```json
{
  "resumeRewrite": {
    "jdProfile": {
      ...
      "mustRequirements": [{"id": "req-1", "requirement": "掌握 RAG 检索", "priority": "HIGH", "keywords": ["RAG", "检索"]}],
      "preferredRequirements": []
    },
    "evidenceBundle": {
      "evidenceIds": ["material-12-1"],
      "items": [{"evidenceId": "material-12-1", "documentTitle": "RAG 项目报告", "sectionName": "检索模块", "snippet": "...", "source": "learning_material", "score": 0.92}],
      "summary": "仅概括候选 evidence 已明确支持的事实。"
    },
    "patches": [{
      "field": "project_experience",
      "suggestedText": "待确认的项目经历文本",
      "reason": "对应 req-1，且引用 material-12-1。",
      "evidenceIds": ["material-12-1"],
      "evidenceQuotes": [{"evidenceId": "material-12-1", "quote": "来自资料片段的精确短语"}],
      "riskFlags": ["NONE"],
      "status": "PENDING_REVIEW"
    }],
    "gapSuggestions": [{"skill": "Agent 编排", "priority": "HIGH", "suggestion": "补充可引用的项目证据"}]
  }
}
```

`patches` 不是 DOCX 操作指令：模型不能输出 `locationRefs`、样式、XML、文件路径或保存动作。事实型补丁必须携带本次 JD 相关 evidence 的精确 `evidenceQuotes`；其中 `project_experience` 还必须引用标题、章节或片段明确表明项目、系统、平台、工程、作品或交付实现的证据。`课程`、`练习`、`作业`、`实验`、`基础` 等教学语义优先否决项目事实，即使同一资料含“开发”“实现”等词也只能形成 `MISSING_EVIDENCE` 缺口，不能自动改写为项目实践。模型生成的候选均为 `DRAFT/PENDING_REVIEW`，模型不能把补丁标记为 `VALIDATED` 或 `CONFIRMED`。`gapSuggestions` 是独立的补强建议，不能被当作简历字段或 DOCX 补丁。

当前生产 Agent 任务只负责生成和审批字段候选；`rag.resume_template` 的 DOCX 解析、校验和确定性导出函数已具备单元测试，但其历史路由尚未注册到当前 FastAPI 公共运行面。因此本次 `OUTPUT` 审批不会直接返回可下载 DOCX；接入生产模板导出 API 需要单独补充公开契约、路由和集成测试。若后续导出链启用，补丁必须再次校验原文 hash、候选 evidence ID、长度与版式契约，且仅 `CONFIRMED` 状态可导出。
...
统一图位于 `ai-python/agents/orchestration/pae_react_graph.py`，使用 `StateGraph` 组织状态、节点和条件边。每次启动或审批恢复都以稳定的 `threadId` 重建当前任务状态；PostgreSQL 的任务、消息、审批、工具观察和摘要是恢复事实源，不依赖进程内 checkpoint。

1. `conversation_title -> context_restore -> task_router`：生成会话标题、恢复当前用户上下文和摘要窗口，再区分只读任务与规划任务。
2. 只读任务依次经过 `memory_prefetch_before_planner -> planner -> memory_prefetch_after_planner`，生成只读计划后进入执行环。
3. 规划任务先由 `planner` 产出计划与完成标准；未批准时进入 `plan_review`，写入 `WAITING_PLAN_REVIEW` 并结束当前图。用户批准后，worker 重新领取任务并以 `plan_approved=true` 恢复。
4. 规划任务经 `resume_rewrite_decision` 判定后，按 `resume_jd_analyzer -> resume_evidence_retriever -> resume_evidence_summarizer -> resume_revision_advisor -> resume_patch_builder -> resume_rewrite_acceptance` 进入简历证据改写子图。JD 分析先产出结构化要求，RAG 节点再使用这些要求检索当前用户私有资料，证据摘要始终保留原始 `evidenceId`、标题、章节、片段、来源和分数；修改建议只能引用候选 evidence，最终补丁准备节点不直接写入 DOCX 或业务数据。
5. 执行环为 `executor -> tool_adapter -> acceptance`：执行器选择白名单动作；工具适配器只经 `LocalAgentGateway` 调用当前用户范围内的只读或已批准变更工具；验收器根据完成标准决定继续下一步、修补或进入回答。
6. 工具失败进入 `repair`。它只能选择有限重试、跳过可降级工具、重新规划或受控失败；联网工具不可用时降级为本地 RAG evidence，资源越权和作用域扩大属于硬失败。`AGENT_GRAPH_RECURSION_LIMIT=24` 防止执行环无限空转。
7. `answer_writer` 产生带 evidence 的最终回答或输出草稿。简历证据改写子图通过验收后直接进入输出草稿，避免在已完成证据检索后再次执行无关 ReAct 工具；规划输出需确认时写入 `WAITING_OUTPUT_REVIEW`。批准后的保存类操作还会进入 `WAITING_CRUD_REVIEW`，并携带审批、操作快照和幂等键。
8. `post_answer_memory` 仅生成 `PENDING_REVIEW` 记忆候选，候选不会自动激活。用户确认后才由 Python 记忆服务写入长期记忆索引。

每个节点通过 `agent_event`、消息投影、工具调用和任务状态写入 PostgreSQL；SSE 将已持久化的 `task`、`agent_event` 和最终 `done` 投影给 React。图执行异常、递归上限、审批拒绝和工具失败都返回受控中文状态，不泄漏资料正文、Token 或连接串。
...
| `resume_jd_analyzer` / `resume_evidence_summarizer` / `resume_revision_advisor` | `qwen-plus` | 依次生成 JD 画像、可引用证据摘要和字段级修改建议 |
...
flowchart TD
    ...
    subgraph PLAN["Planning 子图：PAE + ReAct"]
      ...
      PLAN_REVIEW -->|"拒绝"| PAW["回答节点"]
       RWD -->|"是"| JD["JD 分析子 Agent<br/>结构化岗位要求"]
       JD --> EVIDENCE["学习证据子 Agent<br/>LocalAgentGateway -> RAG"]
       EVIDENCE --> SUMMARY["证据归纳子 Agent<br/>保留 evidenceId 与片段"]
       SUMMARY --> ADVISE["修改建议子 Agent<br/>字段级候选 + evidenceIds"]
       ADVISE --> PATCH["确定性补丁准备<br/>不写 DOCX"]
       PATCH --> REWRITE_ACCEPT["简历候选验收<br/>核对引文、风险与 gapSuggestions"]
       REWRITE_ACCEPT --> PAW
...
6. Planner 检测到简历改写意图时进入 `resume_rewrite_decision -> resume_jd_analyzer -> resume_evidence_retriever -> resume_evidence_summarizer -> resume_revision_advisor -> resume_patch_builder -> resume_rewrite_acceptance` 节点族；该节点族在建议前完成 RAG 检索，所有候选只生成待确认字段补丁，不直接写 DOCX、不保存草稿、不绕过审批。
...
| JD 分析子 Agent | `resume_jd_analyzer` | 从原始 JD 生成带稳定 requirement ID 的岗位画像；不输出 evidence 或文件操作 |
| 学习证据子 Agent | `resume_evidence_retriever` / `resume_evidence_summarizer` | 通过 `LocalAgentGateway` 触发当前用户 RAG，归纳覆盖范围但保留 evidenceId、标题、章节、片段、来源和分数 |
| 修改建议子 Agent | `resume_revision_advisor` | 根据 JD 画像、原简历和 JD 相关 evidence 束生成可微调的字段级候选；事实型候选必须含精确 `evidenceQuotes`，无证据时只能给出缺口与补强建议 |
| 确定性补丁准备 | `resume_patch_builder` / `resume_rewrite_acceptance` | 将 `patches` 与独立 `gapSuggestions` 整理为 `PENDING_REVIEW` 草稿，验收字段、引文和风险后进入 OUTPUT 审批；不直接写文件或数据库 |
...
- 简历证据改写子图不调用 mutation gateway，也不直接写 DOCX；`resume_patch_builder` 只把已验收的字段候选并入规划草稿，后续仍由 `OUTPUT` 审批和受控模板导出链处理。
...
| `AGENT_RESUME_JD_REQUIRED` | 400 | 简历优化任务缺少岗位 JD，无法生成可核验建议 | 否 |
| `AGENT_RESUME_TEXT_REQUIRED` | 400 | 简历优化任务缺少原始简历内容，无法定位字段候选 | 否 |
| `AGENT_RESUME_EVIDENCE_RETRIEVAL_FAILED` | 502 | 简历证据改写子图无法完成当前用户资料检索 | 视错误而定 |
| `AGENT_RESUME_REWRITE_EMPTY` | 422 | JD、原简历和证据不足以形成可审阅字段候选 | 否 |
| `AGENT_RESUME_REWRITE_REJECTED` | 422 | 修改建议未通过 evidence 或字段完整性验收 | 否 |
