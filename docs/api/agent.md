...
```json
{
  ...
  "data": [
    ...
    {
      "toolName": "material_reindex_request",
      "toolType": "MUTATION",
      "requiresReview": true,
      "approvalType": "CRUD",
      "stage": 4,
      "description": "用户审批后重建索引或高精度补跑"
    }
  ]
}
```
...
| `operation_undo_request` | `UNDO` | `agent_operation` | 撤销窗口内恢复 before snapshot |
...
- README 中“第二阶段 Agent 规划”不再声明“未新增接口契约”，并明确 `explicitGrant` 当前只预留。
