...
> 当前运行形态已经切换为纯 Python：`ai-python/app/api/agent.py`、`LocalAgentGateway`、PostgreSQL durable worker 和 Python SSE 是唯一生效实现。本文后续 Java API、Java Tool Gateway、内部 Token 和 Spring 任务编排内容保留为迁移前规划历史，不是当前启动或联调依赖。
