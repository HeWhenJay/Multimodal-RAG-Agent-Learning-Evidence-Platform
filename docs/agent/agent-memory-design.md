...
- 可使用 PostgreSQL 表承载长期记忆；短期热态由 Java Redis adapter 加速，但不作为权威存储。
- Agent 上下文压缩场景下，Redis 只保存 `agent:ctx:{userId}:{taskId}`、`agent:ctx:messages:{userId}:{taskId}`、`agent:sse:{taskId}` 这类短期热态副本。`agent:ctx` 运行中默认 24h、终态默认 7d；`agent:ctx:messages` 默认 7d；`agent:sse` 默认 2h。TTL 过期只影响加速路径，不影响长期恢复；恢复必须以 PostgreSQL `agent_chat_message` 和 `agent_conversation_summary` 为准。
