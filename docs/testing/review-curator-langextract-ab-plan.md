...
- B 臂历史实验使用 `UnicodeTokenizer`、`max_char_buffer=8000`、`extraction_passes=2` 和最多 2 个并发 worker；线上配置已在实验后提升为默认 8 个 worker，A/B runner 仍可独立指定预算复现原结果。
...
本轮 A/B 当时不建议直接接入生产复习卡片生成链路，因为 evidence 映射、近重复和成本三个门槛均未通过。仓库保留官方依赖、严格定位适配器、冻结金标、A/B runner 和离线测试，作为可复现基线。

2026-08-04 产品侧明确选择“完整性优先”，因此线上在保留风险记录的前提下受控启用：仅发布精确定位候选，执行跨 pass 近重复过滤和 topic 轮询，最多选择 32 个知识单元，并要求最终卡片通过 `knowledgeUnitId` 覆盖审计。它仍不能绕过现有卡片问题、答案、evidence 忠实度和 FSRS 写入门禁；生产指标不能反向修改本文件的历史 A/B 结论。
