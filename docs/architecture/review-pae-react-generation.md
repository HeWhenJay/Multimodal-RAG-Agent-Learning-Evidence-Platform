...
复习生成使用 `ai-python/app/review/generation_graph.py` 中的独立 LangGraph，不接入统一 Agent 图。Java 继续只负责业务状态和统一响应；Python 负责 evidence 清洗、默认 Terra 调用、DeepSeek 故障降级、图循环、质量校验与人工处理终态。
