...
CREATE INDEX IF NOT EXISTS idx_learning_material_active_index_job
    ON learning_material(active_index_job_id);
...
CREATE TABLE IF NOT EXISTS rag_index_job (
    ...
    result_json CLOB NOT NULL DEFAULT '{}',
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_rag_index_job_material FOREIGN KEY (material_id) REFERENCES learning_material(id) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_rag_index_job_material_status
    ON rag_index_job(material_id, status, updated_at DESC);

CREATE INDEX IF NOT EXISTS idx_rag_index_job_document
    ON rag_index_job(canonical_document_id, request_version);
...
CREATE INDEX IF NOT EXISTS idx_rag_outbox_status_next
    ON rag_outbox_event(status, next_attempt_at, id);

CREATE INDEX IF NOT EXISTS idx_rag_outbox_lease
    ON rag_outbox_event(lease_until);

CREATE TABLE IF NOT EXISTS rag_consumed_event (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_rag_consumed_job
    ON rag_consumed_event(job_id, consumed_at DESC);
